    ...
    <Card className="rounded-[34px]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">共 {commentTotal} 条评论</p>
        </div>
          ...
          {comments.map((comment) => (
            <div key={comment.id} className="rounded-[26px] border border-[var(--border)] bg-slate-50 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="font-semibold text-slate-900">{comment.userNickname || "匿名用户"}</div>
              ...
              {comment.reply ? (
                <div className="mt-4 rounded-2xl bg-white px-4 py-3 text-sm text-slate-600">
                  <span className="font-semibold text-slate-900">商家回复：</span>{comment.reply}
                </div>
              ) : null}
              <div className="mt-4 text-xs text-slate-400">{formatDateTime(comment.createTime)}</div>
            </div>
          ))}
          <Pagination page={commentPage} pageSize={5} total={commentTotal} onPageChange={setCommentPage} />
        </div>
      ) : (
        <EmptyState title="暂无评价" description="后续可继续增强图片评论等能力。" />
      )}
    </Card>
  );
}
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
          ...
          {images.length > 1 ? (
            <div className="mt-4 grid grid-cols-5 gap-3">
              {images.map((image, index) => (
                <button key={`${image}-${index}`} className={`overflow-hidden rounded-2xl border ${index === imageIndex ? "border-emerald-500" : "border-transparent"}`} onClick={() => setImageIndex(index)} type="button">
        ...
        <Card className="rounded-[34px]">
          <div className="flex flex-wrap items-center gap-3">
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">{product.categoryName || "精选商品"}</span>
            <span className="text-sm text-slate-500">已售 {product.sales}</span>
            <span className="text-sm text-slate-500">库存 {product.stock}</span>
          </div>
          <h1 className="mt-5 text-3xl font-black tracking-tight text-slate-950">{product.name}</h1>
          <p className="mt-3 text-sm leading-7 text-[var(--muted)]">{product.subtitle || "支持购物车加入、立即购买、收藏和评论浏览。"}</p>

          <div className="mt-6 rounded-[28px] bg-rose-50 px-6 py-5">
            <div className="text-sm font-semibold text-rose-700">当前售价</div>
            <div className="mt-2 text-4xl font-black text-rose-600">{formatCurrency(product.price)}</div>
            {product.originalPrice > product.price ? (
              <div className="mt-2 text-sm text-slate-400 line-through">原价 {formatCurrency(product.originalPrice)}</div>
            ) : null}
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            <div>
              ...
              <input min={1} max={Math.max(1, product.stock)} type="number" value={quantity} onChange={(e) => setQuantity(Math.max(1, Math.min(Number(e.target.value || "1"), Math.max(product.stock, 1))))} className="h-11 w-28 rounded-2xl border border-[var(--border)] px-4" />
            </div>
            <Button disabled={product.stock <= 0} onClick={handleAddToCart}><ShoppingCart className="mr-2 h-4 w-4" />加入购物车</Button>
            <Button disabled={product.stock <= 0} onClick={handleBuyNow} variant="secondary">立即购买</Button>
            <Button onClick={handleFavorite} variant="ghost"><Heart className={`mr-2 h-4 w-4 ${favorite ? "fill-current" : ""}`} />{favorite ? "已收藏" : "收藏商品"}</Button>
          </div>

          <div className="mt-8 rounded-[28px] border border-[var(--border)] bg-white p-5">
            <h2 className="text-lg font-black">商品描述</h2>
            <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-[var(--muted)]">{product.description || "暂无详细描述。"}</p>
          </div>
        </Card>
      </div>
